...
export default function SubHexPanel({ hexId, hexData, parentHexId, mapId, onUpdate, onDelete }) {
  ...
  const noteBase = '/api/submaps/' + parentHexId + '/' + mapId + '/notes';
  ...
  useEffect(() => {
    ...
    setConfirming(false);
    fetch(noteBase + '/' + hexId).then(r => r.text()).then(setNote);
  }, [hexId, hexData, noteBase]);
    ...
    <div className="hex-panel">
      ...
      {confirming && (
        <div style={{ fontSize: 11, color: '#e74c3c', marginBottom: 8, background: 'rgba(231,76,60,0.08)', padding: '4px 8px', borderRadius: 4 }}>
          Esto borrara la config y notas de este hexagono.
